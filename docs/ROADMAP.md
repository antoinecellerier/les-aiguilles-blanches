...
- ✅ **Menu scene DynamicTexture consolidation** — Sky gradient + 5 far mountains consolidated into one DynamicTexture (depth 0); snow ground + shadow lines + snowline border into another (depth 3). Near mountains kept separate at depth 2 for bouquetin interleaving; trees/groomer at depth 5+ for animal y-sorting. Objects: 140→101, texture keys cleaned up at both creation and shutdown.
