...
- [ ] **Player retention** — Improve first-time player hook. ✅ Streamlined tutorial (9→4 steps, 14s→1.8s dialogue delay). ✅ Ski run teaser (primary CTA after tutorial). ✅ Cold-open prologue (12s night cinematic for first visit). ✅ Menu atmosphere cycling (10s weather cycle with animal sleep/wake). ✅ L1 polish (shorter piste 60→50, mid-level JP dialogue at 40% coverage, dialogueTriggers system). Remaining: polish weather cycling (fog, heavy storm, golden hour, smoother transitions).
