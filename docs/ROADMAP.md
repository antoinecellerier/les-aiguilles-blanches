...
- [x] **Player retention** — Improve first-time player hook. ✅ Streamlined tutorial (9→4 steps, 14s→1.8s dialogue delay). ✅ Ski run teaser (primary CTA after tutorial). ✅ Cold-open prologue (12s night cinematic for first visit). ✅ Random menu weather (5 moods per visit, storm excluded for first-timers). ✅ L1 polish (shorter piste 60→50, mid-level JP dialogue at 40% coverage, dialogueTriggers system). ✅ Tutorial target raised to 60%. ✅ Daily Runs glow only when unlocked.
