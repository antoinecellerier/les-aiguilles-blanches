...
- **Random Contract** — new seed each run, displayed as 4-6 char shareable code (e.g., `7X2M`).
...
### Ski Mode on Contracts

Every contract has a ski payoff after grooming:
- **Regular contracts**: player skis on their own groomed piste. Slalom gates generated by chance (Blue 30% / Red 50% / Black 70%), with count (6-12) and width (3-5 tiles) scaling with rank.
- **Park contracts**: freestyle trick scoring from kickers/rails/halfpipe. No slalom.
- Groomed tiles saved by seed code for replay.

### Narrative

Contracts are the "resort keeps running" epilogue. Characters appear in short radio-chatter one-liners:
