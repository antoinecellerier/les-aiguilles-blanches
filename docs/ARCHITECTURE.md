...
├── build-tauri.sh          # Build game + launch/package Tauri desktop app (lightweight)
...
│   ├── manifest.json       # PWA web app manifest (offline install)
│   └── icons/              # PWA icons (64–512px)
├── electron/               # Optional desktop wrapper (Electron, ~99 MB AppImage)
...
├── src-tauri/              # Optional desktop wrapper (Tauri v2, ~7 MB .deb)
│   ├── src/lib.rs          # Tauri backend — display modes, fullscreen, quit commands
│   ├── tauri.conf.json     # App config, window settings, bundle targets
│   ├── Cargo.toml          # Rust dependencies
│   └── capabilities/       # Security permissions (shell:open for external links)
├── src/
...
│   │   ├── shareUrl.ts      # Share URL builder/parser/message formatter, clipboard copy
...
│   │   └── CreditsScene.ts  # Cinematic night-slope credits with groomer animation
├── tests/
...
| CreditsScene | Scale resize listener, keyboard listeners, wildlife, trail timer, tweens, children |
...
- **ObstacleBuilder** — Creates obstacles (rocks, trees), interactable buildings (restaurant, fuel station), and decorative chalets. Tracks building footprints for wildlife collision avoidance. Accepts optional `SeededRNG` for deterministic obstacle placement during daily/random runs.
...
- **SlalomGateSystem** — Slalom gate placement and pass/miss detection for ski reward runs. Generates evenly-spaced red/blue pole pairs along the piste with alternating lateral offset. Consecutive gates are clamped to stay within ski physics traversal range while remaining on-piste. Detection triggers when skier Y enters the gate row range; hit = between poles, miss = outside. Visual feedback: ✓/✗ floating text, missed poles dim to 30% alpha. Returns `'hit'`/`'miss'`/`null` per frame for audio integration. Non-punitive — display-only results on level complete. Configured per-level via `slalomGates: { count, width }` in levels.ts.
...
Lightweight desktop packaging via `src-tauri/` using Tauri v2. Uses the OS native webview (WebKit2GTK on Linux, WebView2 on Windows, WebKit on macOS) instead of bundling Chromium. ~7 MB .deb vs ~99 MB Electron AppImage.

### Build Commands

```bash
./build-tauri.sh              # Start Tauri dev mode (run ./dev.sh first)
./build-tauri.sh --build      # Production build for current platform
./build-tauri.sh --debug      # Debug build (faster, unoptimized Rust)
```

Output in `src-tauri/target/release/bundle/` (AppImage, .deb, .rpm on Linux).

### Key Files

- `src/lib.rs` — Tauri backend: `set_display_mode`, `toggle_fullscreen`, `is_fullscreen`, `quit` commands. Sets `WEBKIT_DISABLE_COMPOSITING_MODE=1` on Linux for GPU-accelerated rendering.
- `tauri.conf.json` — App config: window defaults (1280×720, min 800×500), bundle metadata, shell plugin for external links.
- `capabilities/default.json` — Security permissions: `core:default` + `shell:allow-open` for opening GitHub links in system browser.

### Architecture Decisions

- **Desktop API bridge**: `src/types/electron.ts` detects both Electron (`window.electronAPI`) and Tauri (`window.__TAURI_INTERNALS__`). Same exported functions (`isDesktopApp`, `setDisplayMode`, etc.) work with either runtime — no call-site changes needed.
- **Tauri invoke**: Tauri commands are called via dynamic `import('@tauri-apps/api/core')` — tree-shaken out when not running in Tauri.
- **Display modes**: Mapped to Tauri window API (`set_fullscreen`, `set_decorations`, `maximize`). Default is `'windowed'` on all desktop apps.
- **No background throttling**: Unlike Electron, Tauri's native webview doesn't throttle background tabs, so `setBackgroundAudio` is a no-op in Tauri.
- **Icons**: Generated by `electron/generate-icon.cjs` (shared generator), output to `src-tauri/icons/` in Tauri-required formats (PNG, ICO, ICNS).

## PWA (Progressive Web App)

Offline-capable web install via service worker. Users can install directly from the browser — zero runtime overhead.

### How It Works

- **`vite-plugin-pwa`** in `vite.config.ts` generates a Workbox service worker that precaches all static assets (JS, HTML, icons).
- **`public/manifest.json`** defines the web app manifest (name, icons, theme color, standalone display mode).
- **`index.html`** includes `<link rel="manifest">`, `<meta name="theme-color">`, and apple-touch-icon for iOS.
- On build, `dist/sw.js` and `dist/registerSW.js` are generated automatically.

### Offline Support

All game assets are procedurally generated at runtime (Canvas API) — no external images or audio files to cache. The service worker precaches the JS bundles, HTML, and manifest. The game is fully playable offline once installed.

### Installation

- **Android/Desktop Chrome**: Install prompt appears in address bar (requires HTTPS).
- **iOS Safari**: Add to Home Screen from share menu.
- **Desktop Firefox**: Not supported (Firefox removed PWA install support).
...
- `src/scenes/PrologueScene.ts` — Cold-open cinematic shown once for first-time players. Uses `createMenuTerrain` with `isNight: true`, adds night overlay (`0x000022` at 0.45 alpha), sleeping wildlife via `MenuWildlifeController`, and animates a groomer across the slope with directional headlight rectangles on the snow surface. JP dialogue at 4s, auto-advance at 12s, skip on any input. Sets `PROLOGUE_SEEN` in storage. Has shutdown handler for timer/input/wildlife cleanup.
- `src/scenes/CreditsScene.ts` — Cinematic end-game credits that bookend the prologue. Night alpine terrain with groomer driving across the slope (15s, synced to credit scroll), headlights, corduroy trail, sleeping wildlife, and light snow particles. Credits text scrolls through a geometry-masked viewport zone. After scroll completes, JP delivers a farewell line. Localized headings via `t()`, proper nouns in French. All three input methods supported (keyboard/touch/gamepad).
