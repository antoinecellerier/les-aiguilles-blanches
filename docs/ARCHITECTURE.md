...
1. **DynamicTexture consolidation** — Paint many small objects onto a single DynamicTexture at level start. One `drawImage` call replaces hundreds. Used for: off-piste background (tiled pattern via `createPattern('repeat')`), piste snow tiles, access road tiles, background trees/rocks, piste-edge trees, menu sky+far mountains (depth 0) and menu ground+shadow lines (depth 3). L9 went from 876 Images to 65 (-93%). Menu scene: 13 objects (sky rects + mountains + ground) consolidated into 2 DynamicTextures; near mountains and trees kept separate to preserve depth interleaving with animated wildlife
